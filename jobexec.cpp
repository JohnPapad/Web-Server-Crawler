...
void job_executor(char ** boss_worker_ffs, char ** worker_boss_ffs, char ** dirs, unsigned int N, unsigned int n_dirs, int sock_fd)
{
    ...
    {//starting interaction
        ...
        */
        ...
        //char * cmd = NULL;
        if (!send_message(sock_fd, "--> Please type '/search word1 word2 .. word10' to enter query or '/exit' to quit searching.\r\n-> .."))
        {
            cout<<"=> WARNING - Disconnection.."<<endl;
        }

        bool discon;
        char * cmd = read_cmd(sock_fd, discon);
        if (discon == 1)
        {
            cout<<"=> WARNING - Disconnection.."<<endl;
        }
        ...
        else
        {
            ...
            else if (strcmp("/search", cmd_words[0]) == 0)
            {
                if (cmd_nwords < 2)
                {
                    ...
                    if (!send_message(sock_fd, "-! ERROR - wrong number of arguments for SEARCH command !-\r\n\r\n"))
                    {
                        cout<<"=> WARNING - Disconnection.."<<endl;
                    }
                }
                else
                {
                            ...
                            read_search_answers(deadline, N, worker_to_jobexec_fds, sock_fd);
            ...
            else
            {
                ...
                if (!send_message(sock_fd, "-! ERROR - command not found !-\r\n\r\n"))
                {
                    cout<<"=> WARNING - Disconnection.."<<endl;
                }
            }
...
{//get rid of duplicate searching queries
    ...
    for(unsigned int i = 1; i < cmd_words_size; i++)
    {
...
void read_search_answers(unsigned int deadline, unsigned int N, int * worker_to_jobexec_fds, int sock_fd)
{
    alarm(deadline);

    unsigned int success_answers = 0;
    bool none_query_found = 1;
    if (!send_message(sock_fd, "\r\n--> Printing search command's results <--\r\n-------------------------------------\r\n"))
    {
        cout<<"=> WARNING - Disconnection.."<<endl;
    }
    for(unsigned int i = 0; i < N; i++)
        ...
        {//reading all answers from worker
            ...
            {//in order to print answer, worker must not have died during transmission of the answer and timeout threshold must not have been exceeded
                ...
                */
                ...
                if (!send_message(sock_fd, "-> text file's full path: '"))
                {
                    cout<<"=> WARNING - Disconnection.."<<endl;
                }
                if (!send_message(sock_fd, answer_full_file_name_path))
                {
                    cout<<"=> WARNING - Disconnection.."<<endl;
                }
                if (!send_message(sock_fd, "' - line index: '"))
                {
                    cout<<"=> WARNING - Disconnection.."<<endl;
                }

                char li_str[get_number_of_digits(answer_line_index) + 1];
                sprintf(li_str, "%d", answer_line_index);
                if (!send_message(sock_fd, li_str))
                {
                    cout<<"=> WARNING - Disconnection.."<<endl;
                }
                if (!send_message(sock_fd, "'\r\n"))
                ...
                if (!send_message(sock_fd, "-> line's content: '"))
                {
                    cout<<"=> WARNING - Disconnection.."<<endl;
                }
                if (!send_message(sock_fd, answer_line))
                {
                    cout<<"=> WARNING - Disconnection.."<<endl;
                }
                if (!send_message(sock_fd, "'\r\n----------------------------------------------------\r\n\r\n"))
    ...
    if(none_query_found == 1)
    {
        ...
        if (!send_message(sock_fd, "--> None of the words given was found <--\r\n----------------------------------------------------\r\n\r\n"))
        {
            cout<<"=> WARNING - Disconnection.."<<endl;
        }
    }
